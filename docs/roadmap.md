...
| A larger cell table for more programs at once | done | v0.2.99: forty cells, since the supervisor stack is a megabyte (v0.2.97); the disk slot bounds the encoded bytes |
...
| Kernel image headroom | open | 2040 bytes of 260096 at v0.2.99 after the loops folded into one, helpers out of line and the TSS in `.bss`; a size-optimised build outlines routines the ring-3 domains cannot execute |
