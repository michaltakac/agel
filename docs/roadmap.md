...
| Real tail calls in the backend written in Agel: a separate argument area so a tail call reuses the frame whatever the arities | open | today a tail call is a call and a return, so a loop is bounded by the process's stack ([`research/decisions-2026-09.md`](research/decisions-2026-09.md)) |
...
| Fuel inside emitted code, and a conformance test that compiled and interpreted programs charge the same steps | open | the backend's programs are not metered; fuel is the tree-walker's ([`research/jit.md`](research/jit.md)) |
