...
| A program drives the desktop from inside the OS by judgment | done | `scripts/test-drive.sh`: `:drive` shows the loaded program the desktop's own state in the look line and types the command line it decides on, refusing the loops and the halt; `desktop-agent` asks two typed questions a step; `scripts/test-play-bridge.sh`: `agel-play --scene desktop --task TEXT` carries them with the task and the run's history to the provider (a stand-in in CI); live, one three-part task completed in four steps, by hand |
| Perception of window contents as text, actions beyond the desktop's command lines | open | the status line and the focused window's shades are what a driving program sees; command lines are all it types |
| A browser on the OS whose accessibility tree is a state and whose actions are a choice | done | `crates/agel-stdlib/tests/browse.rs`, `scripts/test-browse.sh`, `scripts/test-play-bridge.sh`: `agel/browse`, written in Agel, parses a reader's subset of HTML into numbered elements, acts as a reader, and asks a judge which of the page's links and fields comes next; on the OS a process of the hosted runtime over pages in the data region; through the bridge with `--scene browse`; live, one task on the example site, by hand — no network, no window, no HTML beyond the subset ([`browser.md`](browser.md)) |
| A fetch for the browser through the host, and a window for it | partial | v0.3.1: `(fetch URL NAME)` through the bridge lands a page's text in a file on the OS, and `(plan NAME GOAL)` a model's typed steps; the browser itself still reads the data region, and has no window; a page behind a bot check comes back as the interstitial |
| The desktop usable by a person: clicks that never roll back, a region ready at boot, a sentence that summons the agent | done | `scripts/test-native-workbench.py`: a click on the empty desktop opens the workbench with no rollback, a sentence joins the agent's six cells beside the workbench's nine and drives with the sentence as the request's task; `scripts/test-drive.sh`: `:fs-ls` answers on a fresh boot, Tab and a lone word are told what to do, a sentence drives to the judge's done; `agel-play --attach` and `run-graphics.sh --agent` by hand with the live judge ([`computer-use.md`](computer-use.md)) |
| A larger cell table for more programs at once | done | v0.2.99: forty cells; v0.2.100: sixty-four, of 896 bytes, in a 15.5 KiB workspace slot |
| The pointer the host's own, no capture; a frame never seen half-drawn | done | `scripts/test-desktop-process.py`, `test-native-workbench.py`, `test-play.py`: absolute pointer events through QEMU's `vmmouse` land on the pixel named; `test-native-dock.py`: the device shows what was presented from the back buffer, across a compositor replacement and a reboot ([`native-graphics.md`](native-graphics.md)) |
| The summoned agent starts the game and hands the desktop to its player | done | `:handover NAME STEPS`, the agent's `start-doom` and `play-doom`, `run-graphics.sh`'s persistent disk with the game installed; by hand with the live judge ([`computer-use.md`](computer-use.md)) |
| Kernel image headroom | done | v0.2.100: disk layout v3, a 2 MiB slot loaded above the real-mode megabyte; the image is 283 KiB of 2 MiB |
...
| A run window on the desktop drawing the agent's state, frame time and hardware use | partial | v0.3.2: a chart a model wrote, admitted through the judged gate, paints the route's last ten lengths and the seen share as rectangles from the player's summary; frame time and hardware use are not drawn |
...
| A plan followed step by step by typed questions; a tool written by a model, admitted cell by cell through the judged gate, joined and run as an agent | done | v0.3.2: `:join-file`, `(write ...)`, the builder and the chart; `scripts/test-drive.sh`, `test-play-bridge.sh` ([`release-v0.3.2.md`](release-v0.3.2.md)) |
| A sentence said while agents run is a fact (`task`) and a task line; the DOOM agent asks the judge what it asks for; the reviewer learns from its notes; `:handover ... agents` | done | v0.3.0: `scripts/test-drive.sh`, the film in [`release-v0.3.0.md`](release-v0.3.0.md) |
...
| The agent reads its own metrics and changes its own cells by typed questions, nobody at the keyboard | done | v0.2.99: `review` beside `doom-agent-judge` under `:agents`: one live run, six reviews, two typed questions, the judge chose `nothing` twice; the apply path is tested in `scripts/test-drive.sh` ([`parallel-agents.md`](parallel-agents.md)) |
| Agents side by side on the desktop, each declaring what it needs, a second sentence heard while the first plays | done | v0.2.99: `:agents STEPS`, `NAME-needs` facts (file, window, process, done, after), `scripts/test-drive.sh` |
