...
| The hosted agent runtime running inside a protection domain | partial | `scripts/test-agel-process.sh`: `agel-core` without `std` and the standard library run as a loaded process (`:exec agel -- FILE`), reading a file from its namespace, yielding to the supervisor every 4,096 steps and printing values; its effects are its console and namespace only, and the model adapters, image store and the desktop's effect words have not moved |
...
| A model decides for the in-OS loop, reached through a host bridge | partial | `doom-agent-model.agel` asks with `model-request`; `crates/agel-play` answers each request by calling a provider through `agel-model`; the request/observation and the reply are proven, and driving a whole episode is by hand |
