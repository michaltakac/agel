...
| A session in the OS: the loaded runtime reads the console line by line, each line a transaction in one world, until `:eof` | done | `scripts/test-agel-process.sh`: `:exec agel` answers `PROCESS READING`, lines typed at the desktop's prompt are the program's, a failed transaction leaves the world as it was, `:eof` ends it; the world does not persist past the session |
