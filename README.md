...
**Current release: v0.2.93.** An Agel program drives the desktop from inside the OS: `:drive` steps it with the desktop's own state in the look line — windows, focus, a running process, the terminal's last line — and types the command line it decides on; `desktop-agent` decides by two typed questions a step, carried by `agel-play --scene desktop --task TEXT` to a System One model with the task and the run's history. Live, a three-part task — the help, the listing, the kernel report — was completed in four judged steps. See [`docs/release-v0.2.93.md`](docs/release-v0.2.93.md), [`docs/computer-use.md`](docs/computer-use.md) and [`docs/system-one.md`](docs/system-one.md). What is done, partial and open is in
