...
The current repository is **v0.2.56: the AArch64 kernel boots on QEMU's Raspberry Pi 4 as a flat image entered at EL2 and drives its SD card through an unprivileged SD host controller driver, so programs load and the workspace persists on the card, the rehearsal for the Pi 5; soft shadows, one-pixel edges and press states give the desktop depth; windows move by their header, come to the front when clicked, and hand a process
