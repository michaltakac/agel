...
**Current release: v0.2.99.** Agents side by side, and one that changes the other. Every program in the desktop's world that defines a step is an agent; `:agents` steps each one whose declared needs hold (a file present, a window listening, a process running, another agent done, the clock), hearing a sentence at the prompt between steps. The first pair is the judged DOOM player and `review`, which reads the player's own summary line every step, asks Jev, TypeSafe's System One model, whether the player has stalled and which of its cells should change, and redefines that cell in the shared world, writing what changed and why. DOOM is the instrument; the loop, the needs and the reviewer are general, and [`docs/real-work.md`](docs/real-work.md) is the research and ranking of where an OS like this does real work. The video below (v0.2.97) is Agel and Jev using the OS and playing DOOM. See [`docs/release-v0.2.99.md`](docs/release-v0.2.99.md), [`docs/computer-use.md`](docs/computer-use.md) and [`docs/parallel-agents.md`](docs/parallel-agents.md). What is done, partial and open is in
