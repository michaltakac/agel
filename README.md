...
**Current release: v0.2.98.** The game has a goal: the engine walks its own map and reports, on every state line, the next waypoint on a route to the exit, the route's remaining length and how much of the map has been seen, and the judged DOOM agent steers toward it by arithmetic, asking Jev, TypeSafe's System One model, only what the picture holds (an enemy in view, the danger, the unexplored quadrant of the map), following walls when its rays say it is blocked, using doors, checking the automap on Tab when stalled, and keeping its own metrics in a file on the OS. Honestly labelled, it is a better sensor and a hand-tuned policy, every change made from outside by a person reading the agent's metrics; the plan in [`docs/parallel-agents.md`](docs/parallel-agents.md) makes the next milestone the agent changing its own cells from its own metrics with nobody at the keyboard, then agents side by side, then a lookup through the host and tools it writes for itself, then the OS changed from inside in stages. The video below (v0.2.97) is Agel and Jev using the OS and playing DOOM, recorded live by [`scripts/record-demo.py`](scripts/record-demo.py). See [`docs/release-v0.2.98.md`](docs/release-v0.2.98.md), [`docs/doom.md`](docs/doom.md) and [`docs/system-one.md`](docs/system-one.md). What is done, partial and open is in
...
several programs running at once, each reported as it ends, and a
canvas of pixels a process draws itself, blitted into its window. The
...
./scripts/test-play.sh                   # Agel plays DOOM: the loop an Agel program in the OS, eight steps
./scripts/test-programs.sh               # programs from files: written by Agel, loaded by path or name, /init.agel at boot
