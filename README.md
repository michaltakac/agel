...
**Current release: v0.3.1.** A lookup through the host. An agent on the OS asks, through the same request the judge answers, for a page from the internet and for a plan drawn from it; the bridge fetches the page and reduces it to text, a model reads it once and answers with typed steps, and both land in files on the OS, where the DOOM player reads the plan and asks Jev, TypeSafe's System One model, where it says to head. The OS has no network and runs no model; the lookup is the host's, and the model is used for the one thing no typed question can do. v0.3.0 before it made a sentence a fact: said while agents run, it is the file `task` every agent can read, and the film below shows the player obeying one. DOOM is the instrument; the loop, the needs, the sentence, the reviewer and the lookup are general, per [`docs/real-work.md`](docs/real-work.md). See [`docs/release-v0.3.1.md`](docs/release-v0.3.1.md), [`docs/computer-use.md`](docs/computer-use.md) and [`docs/parallel-agents.md`](docs/parallel-agents.md). What is done, partial and open is in
